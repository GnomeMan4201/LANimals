...
The terminal bridge follows the same rule. `scan arp` uses the approved default CIDR and `scan arp <CIDR>` accepts only a CIDR permitted by the runtime scope configuration.
...
- `tests/test_browser_contract.py` — visible control/route mapping, fail-closed browser mutations, and read-only page initialization.
