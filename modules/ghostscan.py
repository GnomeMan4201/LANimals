...
THREAT_ORGS = [
    "Amazon", "DigitalOcean", "OVH", "Hetzner", "Google", "Alibaba", "Microsoft",
    "LeaseWeb", "Vultr", "Linode", "Scaleway", "DataWeb", "Shodan", "Zenlayer",
    "HostHatch", "G-Core", "Choopa", "Censys", "NetActuate", "Contabo",
    "Tzulo", "Tencent", "Xiaomi", "Alibaba", "Tencent"
]
...
def rdap_lookup(ip):
    try:
        ...
        if r.ok:
            data = r.json()
            org = data.get("name") or data.get("entitySearchResults", [{}])[0].get("vcardArray", [["", []]])[1][1]
            return org
    except Exception:
        return None
