...
def main():
    ...
    console.print(f"[bold cyan]OS:[/bold cyan] {uname.system} {uname.release} ({uname.version})")
    ...
    console.print(f"[bold magenta]CPU:[/bold magenta] Usage: {cpu}% | Cores: {cpu_cores} | Threads: {cpu_threads}")

    # Memory
    mem = psutil.virtual_memory()
    console.print(f"[bold magenta]Memory:[/bold magenta] {mem.percent}% used | {mem.used//(1024**2)}MB / {mem.total//(1024**2)}MB\n")

    # Disk
    disk = psutil.disk_usage('/')
    console.print(f"[bold magenta]Disk:[/bold magenta] {disk.percent}% used | {disk.used//(1024**3)}GB / {disk.total//(1024**3)}GB\n")
    ...
    procs = [(p.info['name'], p.info['memory_info'].rss) for p in psutil.process_iter(['name', 'memory_info'])]
